...
public class Crc16ModbusAlgorithm : ChecksumAlgorithmBase
{
    ...
    /// <inheritdoc/>
    public override ChecksumAlgorithmType Type => ChecksumAlgorithmType.Crc16Modbus;

    /// <inheritdoc/>
    public override string DisplayName => "CRC-16/MODBUS";

    /// <inheritdoc/>
    public override string Description => "CRC-16 MODBUS 校验 (多项式: 0x8005, 初始值: 0xFFFF, 小端序)";
    ...
    public override byte[] Calculate(byte[] data)
    {
        ...
        {
            return [(byte)(InitialValue & 0xFF), (byte)(InitialValue >> 8)];
        }
