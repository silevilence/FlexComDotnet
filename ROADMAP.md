...
- [ ] 🟢 **[P2] CI/CD (Github Actions)**
    - [ ] **触发条件**：推送如V1.0.0这样带版本号的标签时
    - [ ] **执行步骤**
        - 使用Release配置发布生成程序
        - 将生成的程序打包为zip，作为免安装包，文件名为 `FlexComDotnet_V1.0.0.zip` ，其中 V1.0.0部分需要替换为实际版本号
        - 使用Release配置生成 msix 安装包，命名为 `FlexComDotnet_V1.0.0.msix`，同样，V1.0.0部分为实际版本号
        - 生成一个新的 github release，以提交的标签那个commit为基础发布
        - 更新说明使用 `CHANGELOG.md` 中相应版本的内容（使用脚本提取）
        - 将上面的生成的两个文件作为附件加入到release中
    - [ ] **CHANGELOG格式**
        - 一级标题为版本号
        - 二级标题分别为新功能、优化、bug修复，并在标题前加上对应的Emoji
        - 二级标题下，以markdown列表分点描述修改内容，要求内容的描述以用户视角出发，而不是技术人员视角
        - 脚本提取时，根据一级标牌的版本号提取内容，当Changelog中找不到指定的版本号时，要求CI/CI流程失败
